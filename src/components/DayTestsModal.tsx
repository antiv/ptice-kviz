...
const DayTestsModal: React.FC<Props> = ({ show, onHide, startDate, endDate, label }) => {
  ...
  const fetchTestsForPeriod = async () => {
    setLoading(true);
    setError(null);
    
    try {
      // Load all records inside this specific timeframe
      ...
      setError('Greška pri učitavanju testova za odabrani period');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl" centered className={selectedResult ? 'quiz-history-modal-dimmed' : ''}>
      <Modal.Header closeButton>
        <Modal.Title>Urađeni testovi: {label}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {loading ? (
          ...
          <Alert variant="info">Nema sačuvanih testova u ovom periodu.</Alert>
        ) : (
              ...
              <tbody>
                {results.map((result) => (
                  <tr key={result.id}>
                    ...
                    <td className="fw-medium text-break">{result.user_email || 'Nepoznat'}</td>
        ...
        {selectedResult && (
          <Modal show={!!selectedResult} onHide={() => setSelectedResult(null)} size="lg" centered backdrop="static">
            <Modal.Header closeButton>
              <Modal.Title>
                Detalji kviza - {formatShortTime(selectedResult.created_at)}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <div className="mb-3">
                <strong>Korisnik:</strong> {selectedResult.user_email}<br/>
                ...
                <strong>Vrsta okruženja:</strong> {selectedResult.zvanican_test ? 'Zvanični test' : 'Obični kviz'}
              </div>
